...
class TradeExecutor:
    ...
    async def enter_long(self, symbol, entry_price=None):
        ...
        size = self._calculate_trade_size(usdt_balance, entry_price or await self.tracker.get_price(symbol))
        if size <= 0:
        ...
        if result:
            ...
            self.tracker.record_entry(symbol, "long", size, result.get("price") or entry_price)
        else:
